...
public class RegistroVisitantes1 extends javax.swing.JFrame {
    ...
    public RegistroVisitantes1() {
        ...
        btnSeleccionar.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mousePressed(java.awt.event.MouseEvent evt) {
                btnSeleccionar.setBackground(colorPresionado);
            }

            @Override
            public void mouseReleased(java.awt.event.MouseEvent evt) {
                btnSeleccionar.setBackground(colorOriginal);
            }
        });
    }
    private void initListeners(){
        ...
        txtID.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt){
                ...
                if (!Character.isDigit(c) || txtID.getText().length() >= 10) {
                   evt.consume();
                   JOptionPane.showMessageDialog(null, "Solo se permiten números y máximo 10 dígitos.");
        ...
        txtIDUser.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt){
                char c = evt.getKeyChar();
                if (!Character.isDigit(c) || txtIDUser.getText().length() >= 10) {
                    evt.consume();
                }
            }
        ...
        txtEdadUser.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt){
                char c = evt.getKeyChar();
                if (!Character.isDigit(c) ||txtEdadUser.getText().length() >= 3 ) {
                    evt.consume();
                }
            }
        });
...
private void actualizarVisitantes() {
   ...
   private double calcularPrecio() {
    try {
        ...
        if (fecha == null || tipoEntrada.equals("Seleccionar") || horaSeleccionada.equals("Seleccionar")) {
            JOptionPane.showMessageDialog(this, "Por favor completa todos los campos antes de calcular el precio.", "Error", JOptionPane.ERROR_MESSAGE);
            return 0;
        }
        ...
        switch (diaSemana) {
            ...
            case Calendar.SUNDAY:
                JOptionPane.showMessageDialog(this, "No se permiten visitas los fines de semana (Sábado o Domingo).", "Día inválido", JOptionPane.WARNING_MESSAGE);
                return 0;
            default:
                JOptionPane.showMessageDialog(this, "Día inválido seleccionado.", "Error", JOptionPane.ERROR_MESSAGE);
                return 0;
        }
        ...
        JOptionPane.showMessageDialog(this, "Ocurrió un error al calcular el precio: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        return 0;
    }
}
   ...
   private void habilitarPaneles(boolean habilitado) {
    ...
    Color colorActivo = new Color(0, 153, 204);
    Color colorDesactivado = new Color(0, 102, 153);
...
private void guardarAgendaVisitantes() {
    try {
        ...
        List<Agendas> agendasActualizadas = daoA.obtenerTodas();
        mostrarAgendasEnTabla(agendasActualizadas);
...
private void mostrarAgendasEnTabla(List<Agendas> listaAgendas) {
    ...
    int idUsuario;
    try {
        idUsuario = Integer.parseInt(txtIDUser.getText().trim());
    } catch (NumberFormatException e) {
    ...
    private void initComponents() {
        ...
        btnSeleccionar.add(jLabel43, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 6, 70, 20));
